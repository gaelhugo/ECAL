var camera, scene, renderer, dice, diceBis;
var accelerationX,accelerationY;
var vitesseX, vitesseY,vitesseXbis, vitesseYbis;
var goalX, goalY, goalZ, goalXbis, goalYbis, goalZbis;
var counter = 10;
var messageSent = 1;
var myName = '';
var isRemote = false;
...
function init() {
	...
	vitesseX = vitesseXbis = 0;
	vitesseY = vitesseYbis = 0;
	goalX = goalXbis = 0;
	goalY = goalYbis = 0;
    ...
    diceBis = new THREE.Mesh(
    new THREE.BoxGeometry( 600, 600, 600, 1, 1, 1 ),
    new THREE.MeshFaceMaterial( materials ) );

    diceBis.position.x = 400;
...
function reDice(e){
  ...
  goalXbis = goalXbis + Math.PI/2 * Math.round(Math.random()*10);
  goalYbis = goalYbis + Math.PI/2 * Math.round(Math.random()*10);
	vitesseX = vitesseY = vitesseXbis = vitesseYbis = .09;
	messageSent = 0;
	connection.send(JSON.stringify({message:[goalX,goalY,goalZ,goalXbis,goalYbis],diceName:String(myName),type:'rollDice'}));
}

function launchRemoteDice(remoteGoals){
	goalX = remoteGoals[0];
	goalY = remoteGoals[1];
  goalXbis = remoteGoals[3];
	goalYbis = remoteGoals[4];
	vitesseX = vitesseY = vitesseXbis = vitesseYbis = .09;
	messageSent = 0;
	isRemote = true;
}
...
function animate() {
    ...
    diceBis.rotation.x += vitesseXbis;
    diceBis.rotation.y += vitesseYbis;
  ...
  if(diceBis.rotation.x<goalXbis){}else{ vitesseXbis = 0;diceBis.rotation.x = goalXbis;}
	if(diceBis.rotation.y<goalYbis){}else{ vitesseYbis = 0;diceBis.rotation.y = goalYbis;}

	if((vitesseX == 0 && vitesseY == 0 && vitesseXbis == 0 && vitesseYbis == 0) && messageSent==0 ){
		if(!isRemote){
			var val = getValue(dice.rotation.x,dice.rotation.y);
      var valBis = getValue(diceBis.rotation.x,diceBis.rotation.y);
			console.log("valBis",valBis);
			messageSent = 1;
			isRemote = false;
			sendMessage(val,valBis,myName);
		}else{
			isRemote = false;
			messageSent = 1;
		}
		...
		switch(json.type){
			...
			case 'activation':
				if(myName == json.diceName){
					console.log("I ran the dice and get : "+json.message+" AND "+json.messageBis);
				}else{
					console.log("Somebody ran the dice and get : "+json.message+" AND "+json.messageBis);
				}
			break;
...
function sendMessage(value,valueBis,myName){
	connection.send(JSON.stringify({message:value,messageBis:valueBis,diceName:String(myName),type:'activation'}));
}
