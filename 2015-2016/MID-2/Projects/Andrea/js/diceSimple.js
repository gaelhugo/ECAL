...
var pointLight ;
var firstRun = true;
//var allColors = [0x3165ba,0xba3192,0x31bab2,0x5d2747];
...
function init() {
	...
	pointLight = new THREE.PointLight(globalColor);
  ...
  //console.log("GET ::",getValue(0,360));
}
...
function reDice(e){
	...
	messageSent = 0;
  //DON't need to send the info
  //connection.send(JSON.stringify({message:[goalX,goalY,goalZ],diceName:String(myName),assignedColor:String(globalColor),type:'rollDice'}));

  // SEND THE INFO TO CHANGE THE DICE COLOR
  if(!firstRun){
    connection.send(JSON.stringify({type:'changeDiceColor'}));
  }
  firstRun = false;
}
...
function animate() {
	...
	if((vitesseX == 0 && vitesseY == 0 ) && messageSent==0 ){
		if(!isRemote){
			...
			messageSent = 1;
			isRemote = false;
			sendMessage(val,myName,globalColor);
		}else{
			isRemote = false;
			messageSent = 1;
		}
...
if (!window.WebSocket) {
  ...
  console.log("connection ??");
    ...
    console.log('openIT !!');
		connection.send(JSON.stringify({type:'connectTheDice'}));
	}
	...
	connection.onmessage = function(message){
        ...
        if(globalColor == json.assignedColor){
      ...
      */
      ...
      case 'updateColor':
        	globalColor = "#"+json.newColor;
          var col = new THREE.Color(globalColor);
...
function sendMessage(value,myName){
  console.log(globalColor);
	connection.send(JSON.stringify({message:value,diceName:String(myName),assignedColor:String(globalColor),type:'activation'}));
}
